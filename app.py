...
region_data = filtered_df.groupby("المنطقة")["الإيرادات"].sum().reset_index().sort_values(by="الإيرادات")
fig_region = px.bar(
...
with tabs[0]:
    prod_region_data = filtered_df.groupby(["المنتج", "المنطقة"])["الإيرادات"].sum().reset_index().sort_values(by="الإيرادات")
...
with tabs[1]:
    region_prod_data = filtered_df.groupby(["المنطقة", "المنتج"])["الإيرادات"].sum().reset_index().sort_values(by="الإيرادات")
    ...
    fig_region_prod.update_traces(
        hovertemplate="المنطقة: %{x}<br>المنتج: %{customdata}<br
